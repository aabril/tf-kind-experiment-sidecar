...
# Set the Kubernetes API endpoint for events
API_ENDPOINT="https://kubernetes.default.svc/api/v1/namespaces/$NAMESPACE/events?fieldSelector=involvedObject.name=$POD_NAME"

# Watch for events related to the pod
curl -k --header "Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)" "$API_ENDPOINT" | \
while read -r line; do
    ...
    if echo "$line" | grep -q "Reason=Started"; then
        ...
        echo "Launching command: npm run load-policies"
